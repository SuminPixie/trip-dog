...
<body>
  ...
  <main>
    <section id="box">
      ...
      <form name="회원가입" id="join_form" method="post" action="./act/register_input.php" class="needs-validation" novalidate>
        <!-- 아이디 -->
        ...
        <div class="mb-2 mt-4">
          ...
          <textarea name="agree" id="agree" cols="30" rows="10" class="d-block form-control" readonly><?php echo file_get_contents('terms.txt'); ?></textarea>
          <div class="mt-3">
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#id_check').click(function () {
        ...
        // 1차 유효성 검사
        if (!idRegex.test(mb_id)) {
          $('#mb_id').removeClass("is-valid").addClass("is-invalid");
          $('.invalid-feedback').text("6~20자의 영문 또는 숫자만 사용 가능합니다.");
          isIdAvailable = false;
          return;
        }
        // AJAX로 서버에 중복 확인 요청
        $.ajax({
          type: 'POST',
          url: './act/register_check_id.php', // PHP 파일 경로
          data: { mb_id: mb_id },
          success: function (response) {
            if (response.trim() === 'duplicate') {
              $('#mb_id').removeClass("is-valid").addClass("is-invalid");
              $('#mb_id').siblings('.invalid-feedback').text("이미 사용 중인 아이디입니다.");
              isIdAvailable = false;
            } else if (response.trim() === 'available') {
              $('#mb_id').removeClass("is-invalid").addClass("is-valid");
              $('#mb_id').siblings('.valid-feedback').text("사용 가능한 아이디입니다.");
              isIdAvailable = true;
            }
          },
          error: function () {
